...
class WorldFileReader(QtGui.QWidget):
    ...
    def dumpWorldInfo(self, filename, hexSize):
        
        #print "dumping.."        
        
        col_num, row_num = self.x_num, self.y_num = calcHexDimension(self.worldInfo.width, self.worldInfo.height, hexSize, "POINTY")
        ...
        root.appendChild(hex_col_node)
        
        hex_row_node = doc.createElement("hexRows")
        hex_row_node_val = doc.createTextNode(str(row_num))
        hex_row_node.appendChild(hex_row_node_val)
        root.appendChild(hex_row_node)
        
        hex_size_node = doc.createElement("hexSize")
