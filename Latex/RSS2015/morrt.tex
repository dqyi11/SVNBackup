...
Find $ M $ paths $ \sigma^{*} \in \Sigma^{*}  : [0, s] \rightarrow cl(  X \setminus X_{obs} ) $ such that
\begin{itemize}
...
The solve of the subproblem relies on the knowledge of the reference position $ z^{*} $ in the fitness space if we follow the Tchebycheff approach or Boundary intersection approach.
