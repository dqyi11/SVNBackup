...
\begin{equation}
P( \lim_{ i \rightarrow \infty }  x^{\mbox{MORRF}^{*}}_{i} ) = 1.
\end{equation}
\begin{proof}
content...
\end{proof}
...
The cost of the solution of $ i $-th reference tree converges to $ z^{*}_{i} $.
\begin{proof}
...
\label{lem:tree_vex:conv}
	...
	Give enough run time, the cost vertices converge to optimal
\end{lem}

\begin{lem}
\label{lem:sub_tree:conv}
The cost of the solution of $ j $-th subproblem tree converges to $ j $-th subproblem.
\begin{proof}
\end{proof}
\end{lem}
