...
Because $ V(X) $ is real value, by applying equation \eqref{eq:pso_up_linalg_simp} to $ V( X(k+1) ) - V( X(k) ) $, we have
\begin{equation}
...
\item If $ a^{2} \leq 4 \chi $, we have $ - 2 \sqrt{\chi} < a < 2 \sqrt{\chi} $.
...
We use the parameters of $ \Phi^{G} = 2.05 $, $ \Phi^{P} = 2.05 $ and $ \chi = 0.72984 $ in \cite{4223164}, which locates in the margin of stable region in Figure \ref{fig:paramSpace}.
If either $ \Phi^{G} + \Phi^{P} $ or $ \chi $ is increased, the movement of particles become unstable in Figure \ref{fig:bound_case:b} and \ref{fig:bound_case:c}.
Contrastively, if either $ \Phi^{G} + \Phi^{P} $ or $ \chi $  is decreased, the parameters moves to the stable region in the parameter space.
Thus the boundaries of the movement of particles become smaller, which is illustrated in Figure \ref{fig:bound_case:d} and \ref{fig:bound_case:e}.
...
We can see that the parameter selection has an impact on the size of a particle's exploration region and convergence rate.
If all the particles converge quickly and explore small regions, it is easy to have early premature.
If all the particles converge slowly and explore large regions, the swarm is hard to converge.
If we want    
