...
%{\sc Walter, this proof doesn't work because weightings for the first case don't generate a unique solution.  For a four-objective problem, consider consider the cost functions for two paths: $\sigma_a$ and $\sigma_b$.  Suppose the cost functions for these two paths are $c_1(\sigma_a) = \lambda_1 = c_2(\sigma_b)$, ${\rm for}\ j\in{1,2,3}c_j(\sigma_a) = 5$ and $c_j(\sigma_b)=4$.  Both $\sigma_a$ and $\sigma_b$ have the same Tchebycheff cost, $g^{te}(\sigma_a)=g^{te}(\sigma_b)$.  That is why I didn't set the weights for $k\in\overline{P}$ to zero.} 
...
\begin{comment}Let $c(x; i) $ be the minimum cost from the root to the position of vertex $ x $ at iteration $ i$  and $ z^{*}(x) $ be the minimum cost from the start to the position of vertex $ x $.
