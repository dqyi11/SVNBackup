...
%I will firstly calculate $ | c_{k}(x) - z^{*}_{k} | $ for each objective, multiple it with $ k $th value of the vector $ \lambda_m $.
...
Before we start the analysis, we note the following restrictions on the cost functions and obstacle placement required by the RRT$^*$ algorithm~\cite{Karaman.Frazzoli:RSS10}.
...
%{\sc Walter, a conditional cost function $ c_{k} ( \sigma_{1}  \mid \sigma_{2} )$ isn't defined in this paper.  What does it mean?  also, replace $c_k$ with the notation we've been using, $c_{k}$.  Do you mean that if you construct a discontinuous path made up of $\sigma_1$ and $\sigma_2$, the cost for that path $c_k(\sigma_1,\sigma_2)$ is the sum of the costs of the two paths?  If so, assumption 1 becomes:}

%\noindent{\bf Assumption 1.} For a path constructed by composing two other paths (to create a discontinuous path), $\forall k\in K, \sigma_1,\sigma_2\in X_{\rm free}$, $c_k(\sigma_1\circ\sigma_2) = c_k(\sigma_1) + c_k(\sigma_2)$.
...
%{\sc Do the cost functions, which are defined over piecewise linear paths delineated by waypoints, satisfy this condition?  Also, why does the cost function index, $k$, multiply the supremum?  That doesn't make any sense.  I think you mean that there is a constant, call it $\kappa\in \Re^+\cup{0}$, which depends on which cost function you are using, such that the $\kappa(k)$ is the constant.  This means that the difference between the costs of any two paths, measured using $c_k$, can be bounded by the maximum Euclidean distance all equivalent points (the scaling function $\tau$) on each of the paths.}

%{\sc If my assumptions are correct, then assumption 2 becomes:} 

%\noindent{\bf Assumption 2}. For all $k\in K$, the cost function $c_k$ is Lipshitz continuous, that is, for all paths $\sigma_1:[0,1]\rightarrow X_{\it free}$ and $\sigma_2:[0,1]\rightarrow X_{\it free}$, there exists a (cost function dependent) constant $\kappa(k)\in \mathbb{R}_{+}\cup \{0 \}$ such that $ | c_{k} ( \sigma_{1} ) - c_{k} ( \sigma_{2} ) | \leq \kappa(k) \sup_{\tau \in [0,1]} \lVert \sigma_{1} (\tau) - \sigma_{2} (\tau) \rVert $.
...
%{\sc I am trying to prove that because we are checking the minimum cost while adding new vertex.
...
\end{eqnarray}
Because the ``Utopia'' reference vector satisfies,  $ \forall k \in K $, $ z^{\rm utop}_{k} \leq  c_{k}( \sigma )  $.
Therefore, we have
\begin{eqnarray}
\nonumber
...
When $ \sigma^{*} $ is Pareto optimal, there exits two cases, which are $ \exists k ,  c_{k}( \sigma ) = z^{\rm utop}_{k} $ and $ \forall k,  c_{k}( \sigma^{*} ) < z^{\rm utop}_{k} $.

\begin{itemize}
\item $ \exists k ,  c_{k}( \sigma ) = z^{\rm utop}_{k} $ \\
...
\end{itemize}
We can have $ \sigma^{*} $ is a solution to Eq.~\eqref{eq:Tchebycheff}.

\item $ \forall k,  c_{k}( \sigma^{*} ) > z^{\rm utop}_{k} $
We can define the weights as
\begin{equation}
\label{eq:weights:assign}
\forall k \in K, \lambda_{k} = \frac{ \ell_{k} }{ \sum_{j=1}^K \ell_{j} }.
\end{equation}
in which $ \ell_{k} =\frac{1}{| c_{k}(\sigma^*)-z^{\rm utop}_{k} |} $.
\end{itemize}

The Tchebycheff cost for $ \sigma^{*} $ becomes
\begin{equation}
\begin{aligned}
...
Given any solution $ \sigma $, we have
\begin{equation}
...
\begin{aligned}
 ...
 = & \max_{k \in K} \frac{ | c_{k}( \sigma ) - z^{\rm utop}_{k} |  }{ | c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | } \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \\
 = & \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \max_{k \in K} \left| \frac{ c_{k}( \sigma ) - z^{\rm utop}_{k}  }{ c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \\
 = & \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \max_{k \in K} \left| \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  +  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k}  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \\
 = & \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \max_{k \in K} \left| 1 + \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right|
\end{aligned}
\end{equation}
...
Therefore,
\begin{equation}
\exists k' \in K, \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} }  > 0.
\end{equation}
It indicates that
\begin{equation}
...
\begin{aligned}
g^{te} ( \sigma ) 
> &  \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} = g^{te} ( \sigma^{*} ).
\end{aligned}
\end{equation}
It means,
\begin{equation}
...
\end{equation}
Thus, $  \sigma^{*} $ is a solution to Eq. \eqref{eq:Tchebycheff} with weights defined as Eq.\eqref{eq:weights:assign}.

In both two cases, Lemma \ref{lem:exist_weight} is true.

\begin{comment}
Define $P(\sigma^*)=\{ j \ |\  c_{j}(\sigma^*)=z_j^{\rm utop}\}$ and let $\overline{P}=\{1,\ldots,K\} \setminus P$.   
$\forall k\in P$ define  $\ell_k=1$, and $\forall k \in \overline{P}$ define $\ell_k=\frac{1}{|c_{k}(\sigma^*)-z^{\rm utop}_k|}$.   
Normalize the weights, yielding
\begin{equation}
\label{eq:weights:assign}
\forall k \in K, \lambda_k = \frac{\ell_k}{\sum_{j=1}^K \ell_j}.
%\lambda_{k} = \frac{1}{ | c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | } \left( \sum_{j=1}^{K} \frac{1}{ | c_{j} ( \sigma^{*} ) - z^{\rm utop}_{j} | }  \right)^{-1} .
\end{equation}
If $P(\sigma^*) = \{1,2,\ldots, K\}$, then there is only one element in the Pareto set and the result is trivial.  

Otherwise, define the Tchebycheff cost for an arbitrary path $\sigma$ as
\begin{equation}
%\begin{aligned}
 g^{te} ( \sigma )  =  \max_{k \in K} \lambda_{k} | c_{k}( \sigma ) - z^{\rm utop}_{k} |. %\\
%&  =  \max_{k \in K} \frac{ | c_{k}( \sigma ) - z^{\rm utop}_{k} |  }{ | c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | } \left( \sum_{k=1}^{K} \frac{1}{ | c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1}
%\end{aligned}
\end{equation}
%for any given $ \sigma $.
%
...
\begin{eqnarray*}
 g^{te} ( \sigma^{*} ) &=& \max_{k \in K} \lambda_{k} | c_{k}( \sigma^* ) - z^{\rm utop}_{k} | \\
   &=& \max_{k\in K} \frac{\ell_k}{\sum_{j=1}^{K} \ell_j}  | c_{k}( \sigma^* ) - z^{\rm utop}_{k} | \\
   &=& \max_{k\in \overline{P}} \frac{\ell_k}{\sum_{j=1}^K \ell_j}  | c_{k}( \sigma^* ) - z^{\rm utop}_{k} |\\
   &=& \max_{k\in \overline{P}}  \frac{| c_{k}( \sigma^* ) - z^{\rm utop}_{k} |}{ | c_{k}( \sigma^* ) - z^{\rm utop}_{k} |} \frac{1}{\sum_{j=1}^K \ell_j} \\
   &=& \frac{1}{\sum_{j=1}^K \ell_j} \\
%   &=& \\
 %  &=& \left( \sum_{j\not\in P} \frac{1}{ | c_{j} ( \sigma^{*} ) - z^{\rm utop}_{j} | }  \right)^{-1}\\
\end{eqnarray*}
...
\max_{k \in K} \lambda_{k} | c_{k}( \sigma ) - z^{\rm utop}_{k} | \\
=& \max_{k\in K} \frac{\ell_k}{\sum_{j=1}^K \ell_j} | c_{k}( \sigma ) - z^{\rm utop}_{k} | \\
=&  \frac{1}{\sum_{j=1}^K \ell_j} \max_{k\in K} \ell_k | c_{k}( \sigma ) - z^{\rm utop}_{k} |\\
=& \frac{1}{\sum_{j=1}^K \ell_j} \max_{k\in K} \Big\{ \{ \ell_k | c_{k}( \sigma ) - z^{\rm utop}_{k} |\ \big{|} k\in\overline{P}\}\Big.  \\
& \Big.\bigcup \{ \ell_k | c_{k}( \sigma ) - z^{\rm utop}_{k} |\ \big{|} k\in P\}\Big\}\\
%=&  \frac{1}{\sum_{j=1}^K \ell_j} \max_{k\not\in P} \ell_k | c_{k}( \sigma ) - z^{\rm utop}_{k} |\\
=& \frac{1}{\sum_{j=1}^K \ell_j} \max_{k\in K} \Bigg\{ \Big\{ \frac{| c_{k}( \sigma ) - z^{\rm utop}_{k}| }{| c_{k}( \sigma^* ) - z^{\rm utop}_{k} |}\ \big{|} k\in\overline{ P}\Big\} \Bigg. \\
& \Bigg.\bigcup \{ \varepsilon | c_{k}( \sigma ) - z^{\rm utop}_{k} |\ \big{|} k\in P\}\Bigg\}\\
%=&  \frac{1}{\sum_{j=1}^K \ell_j}  \max_{k\in K} \Big\{\{\frac{| c_{k}( \sigma ) - z^{\rm utop}_{k}| }{| c_{k}( \sigma^* ) - z^{\rm utop}_{k} |\ | k\in P\}}\\
%& \bigcup \Big\{ \varepsilon | c_{k}( \sigma ) - z^{\rm utop}_{k} |\ | k\in P\Big\}\Big\}\\
= & \frac{1}{\sum_{j=1}^K \ell_j} \max_{k \in K}\Bigg\{\\
& \Big\{ \Big| \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  +  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k}  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \Big|  \ \big{|} k\in\overline{ P}\Big\}\Bigg.\\
& \Bigg.\bigcup \{ \varepsilon | c_{k}( \sigma ) - z^{\rm utop}_{k} |\ \big{|} k\in P\}\Bigg\}\\
= & \frac{1}{\sum_{j=1}^K \ell_j} \max_{k \in K} \Bigg\{\Big\{\Big| 1 + \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \Big| \ | k\in\overline{ P}\Big\}\Bigg.\\
& \Bigg.\bigcup \{ \varepsilon | c_{k}( \sigma ) - z^{\rm utop}_{k} |\ | k\in P\}\Bigg\}\\
=&  \frac{1}{\sum_{j=1}^K \ell_j}  \max\Bigg\{\max_{k\in \overline{P}}\Big\{\Big| 1 + \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \Big| \Big\} \Bigg.\\
& \Bigg.\max_{k\in P} \Big\{ \varepsilon | c_{k}( \sigma ) - z^{\rm utop}_{k} |\Big\}\Bigg\}.
\end{aligned}
\end{equation}

{\sc Walter, we don't really need the condition that the lambda weights sum to one.  
The math gets cleaner if we don't use that constraint.  Let's drop it.}

{\sc Answer: I think it is still necessary to keep the normalized weight.
It guarantees the uniqueness.
For example, [0.5, 0.5] has same effect with [1.0, 1.0].
If all the weights are normalized, there won't be duplicates. }
...
$
%\end{equation}
If $k'\in \overline{P}$, then 
\begin{equation}
\max_{k \in K} \left| 1 + \frac{ c_{k}( \sigma ) - c_{k} ( \sigma^{*} )  }{  c_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \geq 1
\end{equation}
and Eq.~\ref{eq:exist_weight:leq} becomes 
\begin{equation}
\begin{aligned}
g^{te} ( \sigma ) 
\geq &  \frac{1}{\sum_{j=1}^K \ell_j} = g^{te} ( \sigma^{*} ).
\end{aligned}
\end{equation}
%
%{\sc What if $k'\in P$?}  
%
When $k'\in P$, things get a little more tricky.  
Observe that since  $c_{k'} ( \sigma ) > c_{k'} ( \sigma^{*})$, by the definition of Pareto optimal, and since $c_{k'}(\sigma^*)\geq z_k^{\rm utop}$, by the assumption about the ``Utopia'' vector then $c_k'(\sigma) > z_k^{\rm utop}$.  
Thus, if we set \[\epsilon = \frac{1}{\inf_{\sigma\in X_{\rm free}} \min_{k} c_{k}(\sigma) - z_{k}^{\rm utop}}\] we could guarantee the same bound on the Tchebycheff cost.  
Unfortunately, the sequence of infimum and minimum operators can return a zero, so $\varepsilon$ can be undefined.  
Although there is likely an elegant solution to this, we do not know what this is so we employ a simple trick, namely to redefine our Utopia vector to avoid the divide-by-zero problem.  
Thus, instead of having $z_k^{\rm utop} = \inf_{\sigma\in X_{\rm free}} c_{k}(\sigma)$, we instead set $z_k^{\rm utop} = \inf_{\sigma\in X_{\rm free}} c_{k}(\sigma) - \delta$.  
This guarantees that the maximum of the two maxima in Eq.~\ref{eq:exist_weight:leq} exceeds one, which in turn guarantees that $g^{te} ( \sigma ) 
\geq   \frac{1}{\sum_{j=1}^K \ell_j} = g^{te} ( \sigma^{*} )$.

Thus, for any other path $\sigma$, we can define a weight vector consisting of $\lambda_k$ that guarantees that
$%\begin{equation}
\forall \sigma, g^{te} ( \sigma ) \geq g^{te} ( \sigma^{*} ),
$%\end{equation}
so every Pareto optimal solution $\sigma^*$ can be obtained using the Tchebycheff method.
%Thus, $  \sigma^{*} $ is a solution to Equation \eqref{eq:Tchebycheff} with weights defined as Equation \eqref{eq:weights:assign}.
\end{comment}
...
By Theorem \ref{thm:moo-d:rrt}, we can use the Tchebycheff method to find the Pareto set for the multi-objective path-planning problems.  
%{\sc Walter, I don't know what we get from these citations.  Should we omit them here?}~\cite{4358754}~\cite{miettinen1999nonlinear}.
...
\end{proof}
\end{lem}
%{\sc Walter, it looks like we have replaced $f$ with $c$, introduced an iteration index $i$, and switched from path $\sigma$ to denote our solutions to $x$. 
% Please fix this with a consistent notation and then I'll work on the proof.}

%{\sc ANSWER: here $  c(x, i) $ indicates the cost of vertex $ x $ at iteration $  i $, which is stored at each vertex.
%It is the cost of the path from the root to vertex $ x $.
%}

The solution to a subproblem requires a correct estimation of the reference vector $ \bm{z}^{\rm utop} $. 
To guarantee a correct estimation of this reference vector, we need to prove that the solutions of all the reference trees converge to the optimal.
Since the construction of a reference tree does not depend on any other tree, the construction process is equivalent to the construction of an RRT$^{*}$.
The asymptotic optimality of RRT$^{*}$ in Lemma \ref{lem:tree_vex:conv} therefore applies to the reference tree, so the the cost of the solution converges to the optimal cost with respect to a single objective.

Define $ c^{\rm REF}_{k} ( x ,i ) = F^{\rm REF}_{k} ( x, i  ) $ as the minimum cost from the root to the vertex $ x $ at iteration $ i $ by the $ k $-th reference tree.
\begin{lem}
...
By similar reasoning, given convergence to the Utopia reference vector, we can guarantee that each subproblem tree converges to a point in the Pareto set.
%Then we need to show that after all the reference trees converge to the structure that provides optimal cost, 
%the subproblem tree can also converge to the structure that provides optimal fitness $ g^{*}_{m} ( x \mid \lambda_{m} , \bm{z}^{\rm utop} ) $.
Define $ \bm{c}^{\rm SUB}_{m} ( x ,i ) = F^{\rm SUB}_{\lambda_{m}} ( x, i, \hat{\bm{z}}^{\rm utop} (x) ) $ as the Pareto-optimal cost vector from the root to the vertex $ x $ given $  \hat{\bm{z}}^{\rm utop} $ as reference vector at iteration $ i $ by the $ m $-th subproblem tree.
\begin{lem}
...
\end{equation}
\begin{proof}
%When the reference vector $ \bm{z}^{\rm utop}(x) $ converges, the cost of the minimum cost path from the root to the vertex of position $ x $ in $ m $-th subproblem tree will gradually converge to the optimal by Lemma \ref{lem:tree_vex:conv}.
...
Consider $ F^{\rm SUB}_{\lambda_{m}} ( x, i, \hat{\bm{z}}^{\rm utop} (x) ) $ as a system.
\begin{itemize}
...
%{\sc Walter, the problem now is that you must have the reference trees converge first and then the subproblem trees, but all the trees in the forest are constructed simultaneously.  
%That means that we can't guarantee convergence of the entire algorithm without more work than is done in the proof below.  
%Let's state this as a claim and move to the simulation studies.}

%{\sc Answer: I think this only influences the convergence speed.
%The convergence of RRT* is guaranteed by the ``REWIRE'' process.
%Because of the ``REWIRE'' process, the subproblem trees can converge after the reference trees converged.
%This is like the input-to-state stability of two cascaded input-to-state stable components.
%}

%We are close to being able to state that the MORFF* algorithm converges to points in the Pareto set when using the Tchebycheff method.  
%Unfortunately, the requirement that the reference trees converge before the subproblem trees can converge causes technical problems in the proof of such a condition.  
%Thus, we state the following claim without proof, and provide evidence for this claim in the next section.

%\begin{clm}
%\label{clm:morrt:conv}	
%Given Assumptions~\ref{asmp:additivity}-\ref{asmp:spacing}, the solutions from MORRF$^{*} $ converge to a subset of the Pareto optimal set almost surely.
%\end{clm}
