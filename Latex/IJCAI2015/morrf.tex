...
%$\arg\max_x \sum_{k=1}^{K} \lambda_{k} f_{k} (x) $;
%\item \emph{Tchebycheff}:
%Minimize $ g^{te} (x \mid \lambda , \vec{z}^{\rm utop}) = \max_{1 \leq k \leq K}  \{ \lambda_{k} | f_{k}(x) - z^{*}_{k}  | \} $;
%$\arg\min_x\max_{1 \leq k \leq K}  \{ \lambda_{k} | f_{k}(x) - \vec{z}^{\rm utop}_{k}  | \} $;
%\item \emph{Boundary intersection}:
%Minimize $ g^{bi} (x \mid \lambda , \vec{z}^{\rm utop} ) = d $, subject to $ \vec{z}^{\rm utop} - F(x) = d \lambda $.
%$\arg\min\{d \mid \vec{z}^{\rm utop} - F(x) = d \lambda $\}.
%\end{itemize}
...
%{\sc Walter, a conditional cost function $ c_{k} ( \sigma_{1}  \mid \sigma_{2} )$ isn't defined in this paper.  What does it mean?  also, replace $c_k$ with the notation we've been using, $f_k$.  Do you mean that if you construct a discontinuous path made up of $\sigma_1$ and $\sigma_2$, the cost for that path $f_k(\sigma_1,\sigma_2)$ is the sum of the costs of the two paths?  If so, assumption 1 becomes:}

%\noindent{\bf Assumption 1.} For a path constructed by composing two other paths (to create a discontinuous path), $\forall k\in K, \sigma_1,\sigma_2\in X_{\rm free}$, $f_k(\sigma_1\circ\sigma_2) = f_k(\sigma_1) + f_k(\sigma_2)$.
...
%\noindent{\bf Assumption 2}. For all $k\in K$, the cost function $f_k$ is Lipshitz continuous, that is, for all paths $\sigma_1:[0,1]\rightarrow X_{\it free}$ and $\sigma_2:[0,1]\rightarrow X_{\it free}$, there exists a (cost function dependent) constant $\kappa(k)\in \mathbb{R}_{+}\cup \{0 \}$ such that $ | f_{k} ( \sigma_{1} ) - f_{k} ( \sigma_{2} ) | \leq \kappa(k) \sup_{\tau \in [0,1]} \lVert \sigma_{1} (\tau) - \sigma_{2} (\tau) \rVert $.
...
Any solution of Equation \eqref{eq:Tchebycheff} is Pareto optimal.
\begin{proof}
...
For $ \sigma^{*} $, we have 
\begin{equation}
 g^{te} ( \sigma^{*} ) = \left( \sum_{k=1}^{K} \frac{1}{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1}.
\end{equation}
...
= & \max_{k \in K} \frac{ | f_{k}( \sigma ) - z^{\rm utop}_{k} |  }{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | } \left( \sum_{k=1}^{K} \frac{1}{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1} \\
= & \left( \sum_{k=1}^{K} \frac{1}{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1} \max_{k \in K} \left| \frac{ f_{k}( \sigma ) - z^{\rm utop}_{k}  }{ f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \\
= & \left( \sum_{k=1}^{K} \frac{1}{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1} \\
& \max_{k \in K} \left| \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  +  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k}  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \\
= & \left( \sum_{k=1}^{K} \frac{1}{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1} \\
& \max_{k \in K} \left| 1 + \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right|
\end{aligned}
\end{equation}
...
we have 
\begin{equation}
\max_{k \in K} \left| 1 + \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \geq 1.
\end{equation}
Thus,
\begin{equation}
\begin{aligned}
g^{te} ( \sigma ) 
\geq &  \left( \sum_{k=1}^{K} \frac{1}{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | }  \right)^{-1} \\
\geq & g^{te} ( \sigma^{*} ).
\end{aligned}
\end{equation}
It means,
...
By Lemma \ref{lem:sol_pareto_opt} and Lemma \ref{lem:exist_weight}, we can have Theorem \ref{thm:moo-d:rrt}.

\begin{thm}
\label{thm:moo-d:rrt}
All the solutions to Equation \eqref{eq:Tchebycheff} makes the set of Pareto optimal paths $ \{ \sigma^{*} \} $.
\begin{proof}
...
By Theorem \ref{thm:moo-d:rrt}, we can import the Techbycheff method in solving such a multi-objective optimization problem~\cite{4358754}~\cite{miettinen1999nonlinear}.
Therefore, the multi-objective path planning could be decomposed into a set of subproblems.
...
$
\begin{proof}
By Theorem \ref{thm:moo-d:rrt}, the Tchebycheff method could be imported to decompose the multi-objective optimization problem into a set of subproblems.
