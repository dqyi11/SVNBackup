...
\begin{aligned}
 g^{te} ( \sigma ) = &
 \max_{k \in K} \lambda_{k} | f_{k}( \sigma ) - z^{\rm utop}_{k} | \\
 =& \max_{k\in K} \frac{ \ell_{k} }{ \sum_{j=1}^K \ell_{j} } | c_{k}( \sigma ) - z^{\rm utop}_{k} | \\
 = & \max_{k \in K} \frac{ | f_{k}( \sigma ) - z^{\rm utop}_{k} |  }{ | f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} | } \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \\
 = & \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \max_{k \in K} \left| \frac{ f_{k}( \sigma ) - z^{\rm utop}_{k}  }{ f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \\
 = & \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \max_{k \in K} \left| \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  +  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k}  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| \\
 = & \left( \sum_{j=1}^K \ell_{j}  \right)^{-1} \max_{k \in K} \left| 1 + \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right|
\end{aligned}
\end{equation}

Because $ \sigma^{*} $ is Pareto optimal,
\begin{equation}
\exists k' \in K, f_{k'} ( \sigma ) > f_{k'} ( \sigma^{*} ),
\end{equation}
As $ \forall k,  c_{k}( \sigma^{*} ) > z^{\rm utop}_{k} $,
we have 
\begin{equation}
\exists k' \in K, \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} }  > 0.
\end{equation}
It indicates that
\begin{equation}
\max_{k \in K} \left| 1 + \frac{ f_{k}( \sigma ) - f_{k} ( \sigma^{*} )  }{  f_{k} ( \sigma^{*} ) - z^{\rm utop}_{k} } \right| > 1.
\end{equation}
Thus,
\begin{equation}
\begin{aligned}
