...
\bm{c}( \bm{z} ) = \arg \min_{x} \max_{ k \in K } \lambda_{m, k} | x_{k} - \bm{z}_{k} | 
\end{equation} denote the as the cost vector in $ m $-th subproblem tree given the reference vector $ \bm{z} $.
...
Given Assumptions~\ref{asmp:additivity}-\ref{asmp:spacing} and given the estimated Utopia reference vector $ \hat{\bm{z}} $,
%with $ \bm{z}^{\rm utop} (x) $ for any $ x $ given,
the cost of the solution of $ m $-th subproblem tree converges to the corresponding cost of the $ m $-th subproblem $ \bm{c}_{m}^{*} ( x \mid \hat{\bm{z}} ) $ almost surely.
\end{lem}
...
\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{fig/conv}
