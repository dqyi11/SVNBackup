...
Because the algorithm is aware of the homotopy class of each branch of the tree produced by the RRT$^{*}$ algorithm, the algorithm can enable a type of sliding autonomy for different levels of human intent on the path topology.
...
The next two allow a human to express preference among different path shapes, but also allow the human to trade off between following a desired path shape and optimizing another performance objective.
\begin{enumerate}
\setcounter{enumi}{2}
...
For a bidirectional RRT$^{*}$, the exploration process can still converge to the optimal solution with efficiency improvements
...
It means that a solution, which is generated by connecting the subpaths from two trees with a shared position, converges to the optimal solution that has the shared position as a via constraint.
...
Define a {\em path} in $X$ as a continuous curve parameterized by $s$ as $\sigma(s) : [0,1] \rightarrow X$. 
Define the cost of the path as $ \mbox{\sc Cost} (\sigma) $.  
Give a set of homotopy classes $ H \subseteq \bm{H} ( x_{init}, x_{goal} ) $, in which $ \bm{H} ( x_{init}, x_{goal} ) $ is the set of all the possible homotopy classes defined by $ x_{init} $ and $ x_{goal} $
\footnote{Theoretically there exist infinite homotopy classes,
because of different number of circling an obstacle.
In practice, only a finite number of homotopy classes should be considered.
In this paper, we only look at all the simple homotopy classes, which means we do not consider the homotopy classes that circle any obstacle in the homotopy-based path planning problem.}
.
The goal is to find paths $ \sigma^{*} \in \Sigma^{*}$ such that
\textit{ (a) } $ \forall s \in[0,1], \sigma^*(s) \in X_{\it free} $;
\textit{ (b) } $ \sigma^{*} (0) = x_{init} $ and $ \sigma^{*} (1) = x_{goal} $; and
\textit{ (c) } $ \forall h \in H, \sigma^{*} = \arg \min_{\sigma \in X_{\it free} \land h(\sigma^{*}) = h } \mbox{\sc Cost}  (\sigma) $.
\end{defn} 
The homotopy-based path planning problem defines the optimal solutions in all the homotopy classes in $ H $. 
...
To solve the homotopy-based path planning, we need to figure out an efficient way to identify the homotopy class to which a path belongs.
...
The improved Jenkins' method provides a much simpler initialization process in complex obstacles than the Voronoi diagram, especially in the regions adjacent to the map boundary.
...
Then a center point $ c $ is randomly sampled, 
If the sampled point is located in any line that connects any two $ b_{k} $.
...
Given two paths, their node walk sequences indicate whether they are in the same homotopy class.
It is noticeable that two adjacent regions might be connected with more than one reference frame, which means that two nodes in the topological graph might be connected with more than one edge.
Thus, two paths with the same edge sequence belong to the same homotopy class; but two paths with the same node sequence don't.
...
In \cite{Hernandez201544}, the string is used to identify the homotopy class of each branch in the RRT so that the growth of a tree is constrained in a homotopy class, because starting from the root of the tree to each branch makes a subpath.
...
The only difference is that the string of each branch is updated and there is the \textsc{HomotopyCheck}() to check whether the string of a branch is a substring of one reference string.
...
Both the start tree $ G_{s} $ and the goal tree $ G_{g} $ add a new vertex. 
The \textsc{Connect}() procedure creates a path, which is from the start to goal, by connecting the new vertex with the other tree, which are $ p_{s} $ and $ p_{g} $ correspondingly.
In order to guarantee optimality, a set of near vertices in $ G_{g} $ is provided to find the best vertex to be connected with the new vertex $ x^{new}_{s} $ in $ G_{s} $, and vice versa.
...
Because we are interested with the optimal paths in all the homotopy classes, we need to merge the optimal paths in the string classes that belongs to same homotopy class.
...
\begin{itemize}
	...
	In the Jenkins' method, all the reference frames containing the center point $ c $ show this property.
	...
	A recurring character in a string indicates that the path goes around some obstacles and back to cross a visited region, if the substring between the recurring characters is not a palindrome.
	...
	These types of path topologies might be required in some complicated tasks.
	
	\item \textbf{Palindrome pattern}
	A palindrome pattern is a particular case of recurring characters.
	...
	In minimization problems, it is not useful and can be often ignored, because a palindrome pattern will only increase the cost if the cost function is additive.
	However, in some special scenarios, for example, it is expected to scan a region for a fixed number of times, the palindrome pattern can be used.
...
We restate this as a lemma, and note that it corresponds exactly to that given for Theorem 22 in~\cite{Karaman-RSS-10}.
\begin{lem}
	\label{lem:tree_vex:conv}
	Given Assumptions 1-3 in \cite{Karaman-RSS-10},
	the cost of the path from the root to any vertex in the RRT$^{*}$ converges to the optimal cost almost surely.
\end{lem}
...
If we see the position of $ v $ as a via-point of the paths, we can have the asymptotic optimality with the set of paths has $ v $ as a via-point.
...
\begin{lem}
	...
	\begin{proof}
		...
		It equals the cost-to-go from the position of $ v_{g} $ to the goal.
		Let $ \sigma_{G_{s},G_{g}} (v_{s}, v_{g}) $ be a path concatenated by two sub-paths from $  v_{s} $ and $  v_{g} $. 
		Its cost is written as $ \mbox{\sc Cost} (\sigma_{G_{s},G_{g}} ( v_{s}, v_{g} )) =  c (v_{s}) + c (v_{g}) + c (\mbox{\sc Line} (v_{s}, v_{g}) ) $.
		From Lemma \ref{lem:tree_vex:conv}, we know that $ c (v_{s}) $ and $ c (v_{g}) $ converge to the optimal almost surely.
		$ c (\mbox{\sc Line} (v_{s}, v_{g})  ) $ will also converge, because $ v_{g} $ is selected from a set of near vertices.
		Thus, the cost of $ \sigma_{G_{s},G_{g}} (v_{s}, v_{g}) $ converges to the optimal almost surely. 
		Similarly, we can have the same result in connecting a vertex $ v_{g} $ in $ G_{g} $ with the $ G_{s} $.
	\end{proof}
\end{lem}
...
By the homotopy-aware exploration process, the HA-RRT$^{*}$ can return the optimal paths of different homotopy classes with given homotopy constraints.
In this section, we show the sliding autonomy in different levels of human intent on the topology by using the HA-RRT$^{*}$.
...
By an interactive process, the human can initialize a reference path to represent the topology from his/her intent, which indicates a homotopy class.
...
Because the HA-RRT$^{*}$ only returns one solution that is the optimal path in this homotopy class, a single-directional tree structure is enough to explore the regions.
...
In planning a path for a task, if the human only can tell several regions that should be avoided or several regions that are required to be visited, the HA-RRT$^{*}$ explores the paths that satisfies the defined constraints as well.
...
We define the \emph{positive region} as the region that is required to be visited and the \emph{negative region} as the region that is forbidden to be visited.
Then there is an interactive process for the human to label the positive regions and the negative regions.
Unlabeled regions are neutral, which means no constraint exists.
Figure \ref{fig:region_label} shows an example, by defining the positive region and the negative region in Figure \ref{fig:region_label:label}, in which the red region indicates the negative region and the green region indicates the positive region.
...
In this case, the decomposed region will be split into several smaller regions so that each region is either a positive region or a negative region.
...
The nodes of the negative regions are forbidden to be visited in the search on the topological graph.
...
But there does exist a difference on different topologies.
...
Sometimes inherently the user has a mind of the topology preference, but it is too fuzzy to describe.
...
With the interface of visualizing the optimal path in different homotopy class, the human can select one from them that matches his intent the most.
...
Yet, a bidirectional tree structure can accelerate the convergence rate, like in \cite{starek2014bidirectional}.
When the optimal solutions of more than one homotopy class is to be found, a bidirectional RRT$^{*}$ is required so that each position can have the optimal-to-come sub-path and optimal-to-go sub-path.
By using a bidirectional structure, the asymptotic optimality in each homotopy class can also be guaranteed.

The HA-RRT$^{*}$ enables the sliding autonomy to support different levels of the task supervisor's intent on the topology in planning paths.
...
Also an online identification of human intent might start the workspace reduction earlier so that the efficiency could be enhanced.
