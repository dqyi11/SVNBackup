...
Understanding how the particles move guides how to enhance the algorithm design~\cite{bonyadi2014analysis}, particular the stochastic factors in the velocity update of Equation \eqref{eq:up_vel}.
...
The convergence trajectories can be obtained~\cite{vandenBergh2006937} and the results are compared with empirical results~\cite{6900439}.
...
%In next section, we give the conditions on the ISS of the position-update component, which can be referred to for parameter selection.
%Also we provide how the boundary is given the states of the personal best and the global best, which are the inputs to the position-update component.
...
% We assume that all the dimensions are within same parameters.
\begin{equation}
