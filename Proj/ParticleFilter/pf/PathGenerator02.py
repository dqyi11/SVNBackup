...
class PathGenerator(object):
    ...
    def generate(self, length_num, init_pos, sys_input):
        ...
        if self.oNoise > 0:
            obs_noise = np.random.normal(loc=0, scale=self.oNoise, size = length_num*4)
        else:
            obs_noise = np.zeros(length_num*4)
        ...
        for i in range(length_num):
            ...
            n_obs1[i,0], n_obs1[i,1] = oA + obs_noise[i*4], oB + obs_noise[i*4+1]
            n_obs2[i,0], n_obs2[i,1] = oA + obs_noise[i*4+2], oB + obs_noise[i*4+3]
            
            cX, cY = pos[i,0], pos[i,1]

        return pos, n_obs1, n_obs2
