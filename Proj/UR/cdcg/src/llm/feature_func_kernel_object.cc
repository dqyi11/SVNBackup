...
Feature_Func_Kernel_Object( const bool& invert,
                        const unsigned int& objectType ) : h2sl::Feature( invert ),
...
Feature_Func_Kernel_Object::
Feature_Func_Kernel_Object( const Feature_Func_Kernel_Object& other ) : h2sl::Feature( other ),
...
value( const unsigned int& cv,
  ...
  if( func_kernel != NULL ){
    if( func_kernel->object().type() == _object_type ){
      return !_invert;
    }
  }
  return false;
