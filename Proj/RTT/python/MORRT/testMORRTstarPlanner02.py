...
if __name__ == '__main__':
    ...
    def calcCost1(currentPos, referencePos):
        ...
        for x in range(x1, x2):
            ...
            points.append(coord)
            if coord[1] >= obj1Vals.shape[0] or coord[0] >= obj1Vals.shape[1]:
                continue
            cost += obj1Vals[int(coord[1]),int(coord[0])]/255.0
            error -= abs(dy)
            if error < 0:
                y += ystep
                error += dx
    ...
    def calcCost2(currentPos, referencePos):
        ...
        for x in range(x1, x2):
            coord = (y, x) if is_steep else (x, y)
            points.append(coord)
            if coord[1] >= obj2Vals.shape[0] or coord[0] >= obj2Vals.shape[1]:
                continue
            cost += obj2Vals[int(coord[1]),int(coord[0])]/255.0
            error -= abs(dy)
            if error < 0:
                y += ystep
                error += dx
