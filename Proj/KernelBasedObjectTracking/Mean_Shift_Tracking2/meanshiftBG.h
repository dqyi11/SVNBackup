...
private:
	...
	void computeNormalizedBackgroundHist(const Mat& backgroundRegion, const Rect& foregroundRegion, Mat& hist, int N);
	...
	void computeBackgroundWeightedHistogram(const Mat& frame, const Rect& foregroundRect, Mat srcDst, const int numBins);

	void computeBackgroundWeights(const Mat& hist_background, Mat backgroundWeights, const int numBins);
	// current frame
	Mat currentFrame;
