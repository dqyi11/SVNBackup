...
img_edge = canny(img_gauss, 10, 40)


img_hough = houghCircle(img_edge, [detect_radius])[:,:,0]

centers = findLocalMax(img_hough, 0.35)
#centers = findLocalMaxUsingDifferentThreshold(img_hough, 0.6, 0.15, detect_radius)
...
cv2.imwrite(img_filename+"-01-2-"+str(detect_radius)+".png", cimg)
cv2.waitKey(0)
