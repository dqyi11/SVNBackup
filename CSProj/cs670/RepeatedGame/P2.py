...
if __name__ == '__main__':
    ...
    cnt = 0;
    for line in open('100-run-0.75.txt'):
        data = line.split(",")  
    ...
    cnt = 0;
    for line in open('100-run-0.9.txt'):
        data = line.split(",")  
    ...
    cnt = 0;
    for line in open('100-run-0.99.txt'):
        data = line.split(",")  
    ...
    #ax.set_title('Weasel vs all agents: scores by P=0.7, 0.9 and 0.99 of 100 stages')
    xTickMarks = [str(s) for s in agentName]
    ax.set_xticks(ind+width)
    xtickNames = ax.set_xticklabels(xTickMarks)
    plt.setp(xtickNames, rotation=45, fontsize=10)
    
    ax.legend( (rects1[0], rects2[0], rects3[0]), ('P=0.7', 'P=0.9', 'P=0.99') )
