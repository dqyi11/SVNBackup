...
if __name__ == '__main__':
    ...
    prob1 = pulp.LpProblem("Battle of the Sexes 1", pulp.LpMaximize);
    ...
    prob1 += 2.0 * w_lw >= U1, "Constraint 1"
    prob1 += 1.0 * w_wl >= U1, "Constraint 2"
    prob1 += w_lw + w_wl == 1.0, "Probability Constraint"
    ...
    prob2 = pulp.LpProblem("Battle of the Sexes 2", pulp.LpMaximize);
    
    U2 = pulp.LpVariable("Utility 2", cat='Continuous');
    h_lw = pulp.LpVariable("Wife LW", lowBound=0, upBound=1.0, cat='Continuous', e=None);
    h_wl = pulp.LpVariable("Wife WL", lowBound=0, upBound=1.0, cat='Continuous', e=None);
    
    prob2 += U2, "Objective Function"
    prob2 += 1.0 * h_lw >= U2, "Constraint 1"
    prob2 += 2.0 * h_wl >= U2, "Constraint 2"
    prob2 += h_lw + h_wl == 1.0, "Probability Constraint"
