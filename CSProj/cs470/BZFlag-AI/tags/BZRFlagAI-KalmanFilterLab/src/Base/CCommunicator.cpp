...
#include <stdlib.h>
#include <string.h>
#include "../GridFilter/COccGridState.h"
...
bool CCommunicator::occgrid(int index, COccGridState & state)
{
	vector<string> v;
	// Set the desired angular velocity to the specified value.
	char char_buff[20];
	...
	string occGridStateStr="";

	v = readArr();
	while(0 != v.at(0).compare(END))
	{
	...
	ret = state.init(occGridStateStr);
