...
namespace WingmanPathPlanning.Hexagonal
{
	...
	public class HexagonalMap
	{
		...
		private Hexagonal.HexagonalMapState mapState;
        private MapStateManager mapStateMgr;

        public enum Direction { EAST, WEST, NE, NW, SE, SW, STAY };

        Direction[] directionList = { Direction.EAST, Direction.WEST, 
		...
		public Hexagonal.HexagonalMapState MapState
		{
			get
			{
				return mapState;
		...
		private void Initialize(int width, int height, int side, Hexagonal.HexOrientation orientation, int xOffset, int yOffset)
		{
			...
			this.mapState = new HexagonalMapState();

			float h = Hexagonal.CalcUtil.CalculateH(side); // short side
			float r = Hexagonal.CalcUtil.CalculateR(side); // long side
        ...
        {
            return mapStateMgr.GetEntropyColor(x, y);
        }
