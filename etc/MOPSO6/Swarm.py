...
class Swarm(object):
    ...
    def __init__(self, particleNum, particleDimension):
        ...
        self.gbSet = PersonalBestSet(-1, self.particleDimension);
    ...
    def update(self):
        ...
        for i in range(self.particleNum):
            ...
            if np.amax(minObj) <= 0:
                ...
                self.gbSet.addObs(p.pos, True);
            else:
                p.nondominated = False;
                p.pbSet.addObs(p.pos, False);
                self.gbSet.addObs(p.pos, False);
        ...
        for i in range(self.particleNum):
            ...
            if p.nondominated == True:
                '''
                p.pbSet.learnProb();
                sample = self.getRandomPos();
                while(p.pbSet.acceptSample(sample)
                {
                    sample = self.getRandomPos();
                }
                p.localbestPos = sample;
            ...
            if p.nondominated == True:
                '''
                sample = self.getRandomPos();
                while self.gbSet.acceptSample(sample) == False:
                    sample = self.getRandomPos();
                
                p.globalbestPos = sample;
                if p.globalbestPos == None:
                    p.globalbestPos = self.globalbestFitnessPos; #self.getRandomPos();
                '''
                
                
                globalbestIdx = int(np.random.random() * len(self.nondominatedSet));
                p.globalbestPos = self.nondominatedSet[globalbestIdx].pos;
                
                personalbestIdx = int(np.random.random() * len(self.nondominatedSet));
                p.personalbestPos = self.nondominatedSet[personalbestIdx].pos;
