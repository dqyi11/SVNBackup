...
class PerformanceLogger(object):
    ...
    def log(self):
        ...
        averageDistance = calcAverageDist(self.swarm.nondominatedParticles, self.swarm.paretoPos);
        ...
        self.histAverageDistance.append(averageDistance);
    ...
    def plot(self, path=None):
        ...
        datalen = len(self.histAverageDistance);
        x_data = np.arange(datalen);
        ax2.plot(x_data, self.histAverageDistance);
        
        title2 = "Average distance @ " + str(datalen);
    ...
    def dump(self, filename, path):
        ...
        f2 = open(path+"\\"+filename+"-AD.txt", "w");
        for dist in self.histAverageDistance:
            f2.write(str(dist)+" ");
        f2.close();
