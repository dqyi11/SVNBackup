...
class Swarm2D(Swarm):
    ...
    def initReferenceSet(self, loadFromFile=False, nondomSetFile=None, domSetFile=None):        
        ...
        self.worldRefSet = [];
        idxCnt = 0;
        for x in np.arange(self.worldrange[0][0], self.worldrange[0][1], 0.05):
            for y in np.arange(self.worldrange[1][0], self.worldrange[1][1], 0.05):  
        ...
        for x in np.arange(self.worldrange[0][0], self.worldrange[0][1], 0.1):
            for y in np.arange(self.worldrange[1][0], self.worldrange[1][1], 0.1):          
                self.worldRefSet.append([x, y])
        #print self.worldRefSet
        self.worldRefSet = np.array(np.matrix(self.worldRefSet));
        
        #print self.worldRefSet.shape
        
        self.gbSet.setRefSet(self.worldRefSet)
        for p in self.particles:
            p.pbSet.setRefSet(self.worldRefSet)
    ...
    def plot(self, count, path=None):
        ...
        if self.gbSet.prob != None:
            ...
            print len(self.gbSet.prob)
            for i in range(len(self.gbSet.prob)):
                if self.gbSet.prob[i] > self.gbSet.threshold:
                    estPosX.append(self.gbSet.candX[i,0]);
                    estPosY.append(self.gbSet.candX[i,1]);
                else:
                    estNegX.append(self.gbSet.candX[i,0]);
                    estNegY.append(self.gbSet.candX[i,1]);
                    
            ax3.plot(estPosX, estPosY, 'sr')
            ax3.plot(estNegX, estNegY, 'sb')
            
            title31 = "global est - " + str(count) + "run";
            ax3.set_title(title31)
            filename31 = title31 + ".png";
            #print filename31
            
            if path != None:
                filename31 = path + "\\" + filename31
            plt.savefig(filename31);
        ...
        if self.gbSet.kernel != None:
            ...
            y_pred = self.reponseFunc(y_pred)
