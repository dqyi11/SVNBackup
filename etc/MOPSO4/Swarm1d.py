...
class Swarm1D(Swarm):
    ...
    def getCentroidOfNondominatedSet(self):        
        centroid = np.matrix((1,1), np.float);
    ...
    def plot(self,count,path=None):
        ...
        #ax1.plot(self.paretoPeak, 1, 'o', color='yellow');
        ...
        ax2.plot(nondomfit1, nondomfit2, '.', color='green');
        ...
        if len(self.histSwarmCentroid) > 0 and self.showSwarmCentroid == True:            
            ...
            ax11.set_ylabel("pos");
            title11 = "1D centroid hist " + str(count) + "run";
