    ...
    def update(self):
        ...
        for p in self.particles:
            ...
            for d in range(self.particleDimension):
                ...
                globalForce = self.phi2 * u2 * (self.particles[self.globalbestAgentIdx].localbestPos[0,d] - p.pos[0,d]);
                
                p.vel[0,d] = self.inertia * (p.vel[0,d] + localForce + globalForce)
                if p.vel[0,d] == 0.0:
                    print "d " + str(d) + " index " + str(p.index);
                    
                temp1 = p.pos[0,d];
                p.pos[0,d] = p.pos[0,d] + p.vel[0,d];
                temp2 = p.pos[0,d];
                if temp1 == temp2:
                    print "unchanged " + str(p.vel[0,d]);
